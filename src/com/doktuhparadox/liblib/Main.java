...
public class Main {
    ...
    public static void main(String[] args) throws FileNotFoundException, /*Won't happen*/ ParseException {
        ...
        while (true) {
            ...
            if (f.exists()) { //Make sure the file exists
                ...
                Map<String, String> previousWords = new HashMap<>(4);

                for (int i = 0; i < rawLibWords.length; i++) {
                    String word = rawLibWords[i];

                    if (word.matches("%\\w*(\\-\\d)?%(\\p{Punct}+)?")) { //It's a placeholder asking for a replacement
                    ...
                    } else if (word.matches("%\\$\\w*(\\-\\d)?%(\\p{Punct}+)?")) { //It's a placeholder asking for a word from the map
                        if (previousWords.containsKey(cleanedForMapKey(word))) {
                            rawLibWords[i] = previousWords.get(cleanedForMapKey(word)); //Put
                        } else {
                            throw new ParseException("Unresolved reference: " + word, i);
                        }
                    }
                }
